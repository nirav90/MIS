<%@page import="com.mis.formBean.SumInsuredForm"%>
<%@page import="java.util.ArrayList"%>
<%@ taglib uri="/struts-tags" prefix="s"%>


<div id="Credit Insurance" class="t">

	<table width="100%">

		<tr style="background-color: darkblue">
			<td colspan="8" style="color: white"><b>
					<center>Sum Insured Details</center>
			</b></td>
		</tr>

		<tr>
			<td rowspan="2"><b><center>Particulars</center></b></td>
			<td colspan="7">
				<center>
					<b>Details</b>
				</center>
			</td>
		</tr>

		<tr>
			<td>Sum Insured in INR</td>
			<td>Discount(%)</td>
			<td>Premium Before Terro and Tax</td>
			<td>Terrorism Premium</td>
			<td>Premium Net of Service Tax</td>
			<td>Service Tax</td>
			<td>Total Premium Paid</td>
			<td></td>
			<td></td>
			<td></td>
		</tr>
		<s:iterator value="sumInsuredForms">
			<tr>

				<td><b><s:label theme="simple"> Anual Turnover Limit </s:label></b></td>
				<td><s:label name="sumInsINR" theme="simple" /></td>
				<td><s:label name="discount" theme="simple" /></td>
				<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
				<td><s:label name="terrorismPremium" theme="simple" /></td>
				<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
				<td><s:label name="serviceTax" theme="simple" /></td>
				<td><s:label name="totalPremiumPaid" theme="simple" /></td>
			</tr>
		</s:iterator>
		<tr>
			<td><b><s:label theme="simple"> PBL </s:label></b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td><s:label name="discount" theme="simple" /></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td><s:label name="serviceTax" theme="simple" /></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>
		<tr>
			<td><b><s:label theme="simple"> PLL </s:label></b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td><s:label name="discount" theme="simple" /></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td><s:label name="serviceTax" theme="simple" /></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>

		<tr>
			<td><b>Total</b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>


		<tr style="background-color: darkblue">
			<td colspan="8" style="color: white"><b><center>Other
						Coverages</center></b></td>
		</tr>

		<tr style="background-color: darkblue">
			<td colspan="8" style="color: white"><b><center>Sum
						Insured Details</center></b></td>
		</tr>

		<tr>
			<td rowspan="2"><b><center>Particulars</center></b></td>
			<td colspan="7"><center>
					<b>Details</b>
				</center></td>
		</tr>
		<tr>
			<td>Sum Insured in INR</td>
			<td>Discount (%)</td>
			<td>Premium Before Terro and Tax</td>
			<td>Terrorism Premium</td>
			<td>Premium Net of Service Tax</td>
			<td>Service Tax</td>
			<td>Total Premium Paid</td>
			<td></td>
			<td></td>
		</tr>
		<tr>
			<td><b> Anual Turnover Limit</b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td><s:label name="discount" theme="simple" /></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td><s:label name="serviceTax" theme="simple" /></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>
		<tr>
			<td><b> PBL </b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td><s:label name="discount" theme="simple" /></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td><s:label name="serviceTax" theme="simple" /></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>
		<tr>
			<td><b> PLL </b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td><s:label name="discount" theme="simple" /></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td><s:label name="serviceTax" theme="simple" /></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>

		<tr>
			<td><b>Total</b></td>
			<td><s:label name="sumInsINR" theme="simple" /></td>
			<td></td>
			<td><s:label name="premiumBeforeTerroAndTax" theme="simple" /></td>
			<td><s:label name="terrorismPremium" theme="simple" /></td>
			<td><s:label name="premiumNetOfServiceTax" theme="simple" /></td>
			<td></td>
			<td><s:label name="totalPremiumPaid" theme="simple" /></td>
		</tr>

		<tr style="background-color: darkblue">
			<td colspan="8" style="color: white"><center>
					<b>Brokerage Details</b>
				</center></td>
		</tr>

		<tr>
			<td colspan="4">Insurance Company Name</td>
			<td colspan="2">Percentage(%)</td>
			<td colspan="2">Amount</td>
		</tr>

		<tr>
			<td colspan="4"><select name="insurance_company_name">
					<option>Bharti Axa General Insurance Co</option>
					<option>TATA Aig General Insurance Co Ltd</option>
					<option>HDFC Ergo General Insurance Co Ltd</option>
					<option>BAJAJ Alliance General Insurance Co Ltd</option>
			</select></td>

			<td colspan="2"><s:label name="" onchange="GetAmount()"
					theme="simple" /></td>

			<td colspan="2"><s:label name="" theme="simple" /></td>

		</tr>


	</table>
</div>
